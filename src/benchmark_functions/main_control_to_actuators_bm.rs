...
use crate::actuator::sprayers::{operation_suspend};
...
use crate::benchmark_functions::actuators_bm::execute_actuators;
...
pub fn bm_main_control_to_actuators() {
    ...
    thermal_sender.send(30.to_string()).unwrap();
    gps_sender.send(gps).unwrap();
    ultra_sensor_sender.send(ultra_data).unwrap();
    batt_sender.send(100.to_string()).unwrap();
